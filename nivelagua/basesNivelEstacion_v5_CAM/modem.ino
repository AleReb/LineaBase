...
bool sendCache() {
  ...
  if (!file) {
    Serial.println("No se pudo abrir cache.csv");
    return false;
  }
  ...
  while (leerLinea(file, linea, sizeof(linea))) {
    ...
    // ---- 1) PARSEAR CSV (solo las N variables) ----
    char* tok = strtok(linea, ",");
    char* stamp = tok;  // timestamp

    // We expect totalVariables-1 floats after the timestamp
    const uint8_t numValues = totalVariables - 1;  //helper
    float parsed[numValues];                       //
    uint8_t col = 0;
    while (col < numValues && (tok = strtok(nullptr, ","))) {
      parsed[col++] = atof(tok);
    }
    if (col != numValues) {
    ...
    // Copy into dataToSend[]
    for (uint8_t i = 0; i < numValues; i++) {
      dataToSend[i] = parsed[i];
    }
    ...
    p += sprintf(p, "http://api-sensores.cmasccp.cl/insertarMedicion?"
                    "times=%s",
                 stamp);
